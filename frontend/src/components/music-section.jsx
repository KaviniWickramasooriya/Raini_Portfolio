      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-12 grid items-start gap-12 lg:grid-cols-2">
          <div>
            ...
            <button
              type="button"
              className="mt-4 inline-flex h-11 w-full items-center justify-center gap-2 rounded-[var(--radius-md)] bg-accent px-3 font-sans text-xs font-medium uppercase tracking-[0.18em] text-accent-fg transition-opacity hover:opacity-90"
            ...
            {openEmbed ? (
              <div className="relative mt-4 overflow-hidden rounded-[var(--radius-lg)] bg-surface shadow-[var(--shadow-border)]">
                <div className="flex items-center justify-between border-b border-line bg-elevated px-4 py-2.5">
                  <span className="font-sans text-xs uppercase tracking-[0.2em] text-accent">Spotify Catalogue</span>
                  <button
                    type="button"
                    onClick={() => setOpenEmbed(false)}
                    className="inline-flex items-center gap-1.5 rounded-[var(--radius-xs)] bg-surface px-2.5 py-1 text-xs text-fg transition-colors hover:bg-accent hover:text-accent-fg"
          ...
          <div>
            <ol className="max-h-[580px] overflow-y-auto divide-y divide-line rounded-[var(--radius-xl)] bg-surface/70 p-2 shadow-[var(--shadow-border)] backdrop-blur-sm">
              {tracks.map((t, i) => (
                <li key={t.id}>
                  <a
                    href={t.spotify}
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 pr-1">
                      <span className="block truncate font-serif text-sm sm:text-lg leading-tight text-fg">
                        {t.title}
                      </span>
                      <span className="block truncate text-[0.65rem] sm:text-xs text-muted">{t.note}</span>
                    </span>
                    <span className="hidden font-sans text-xs text-subtle sm:inline">{t.year}</span>
                    <span className="font-sans text-[0.7rem] sm:text-xs tabular-nums text-subtle shrink-0">{t.duration}</span>
                  </a>
                </li>
              ))}
            </ol>
